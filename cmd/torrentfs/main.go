...
func exitSignalHandlers(tfs *torrentfs.TorrentFS, unmount func()) {
...
func addTestPeer(client *torrent.Client) {
	for _, t := range client.Torrents() {
		t.AddPeers([]torrent.PeerInfo{{Addr: args.TestPeer}})
	}
}
...
func mainErr() error {
		...
		for ev := range dw.Events {
			...
			case dirwatch.Added:
				if ev.TorrentFilePath != "" {
					if _, err := client.AddTorrentFromFile(ev.TorrentFilePath); err != nil {
						log.Printf("error adding torrent from file %q: %v", ev.TorrentFilePath, err)
					}
				} else if ev.MagnetURI != "" {
					if _, err := client.AddMagnet(ev.MagnetURI); err != nil {
	...
	u, err := tfs.Mount(args.MountDir, &ogtorrentfs.Backend{})
	if err != nil {
	...
	defer unmount()
	go exitSignalHandlers(tfs, unmount)
	...
	logger.Levelf(log.Debug, "torrentfs mounted at %s", args.MountDir)
	// Block until destroyed (signal handler calls unmount which closes the conn).
	<-tfs.Destroyed()
	return nil
}
